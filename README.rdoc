...
You will need to obtain the One Drive details which are necessary for integration with Microsoft's services. You must use the Microsoft Graph Explorer and make sure you are logged in, as by default it uses a test account. Within the Microsoft Graph Explorer you need to visit `https://graph.microsoft.com/v1.0/me?$select=mySite` which will return your `mySite` address.

This needs to be put into your environment variable with the following appended `_api/v2.0` as: `ONEDRIVE_MYSITE`.
The rest of the One Drive credentials must be populated and can be achieved by registering a Web Application through Microsoft's developer portal.
